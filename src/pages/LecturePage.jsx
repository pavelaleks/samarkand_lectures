  ...
  useEffect(() => {
    const loadLecture = async () => {
      ...
      try {
        ...
        // lecture.contentFile уже содержит путь типа: src/data/alternative/лекции/1.html
        ...
        // Учитываем base path для разработки
        const basePath = import.meta.env.BASE_URL || '/samarkand_lectures/'
        
        // Убираем base path из начала, если он уже там есть
        if (filePath.startsWith(basePath)) {
          filePath = filePath.substring(basePath.length - 1) // -1 чтобы оставить /
        }
        
        // В dev режиме Vite обслуживает файлы из src/data напрямую
        // В production файлы будут в dist/src/data
        console.log('Загрузка файла:', filePath, 'Base URL:', basePath)
